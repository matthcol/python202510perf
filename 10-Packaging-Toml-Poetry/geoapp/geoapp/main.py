...
def play_with_shapes_matching():
    ...
    cpt_f = ColoredPoint(name="F", x=np.float64(0.5), y=np.float64(0.8), color="red")
    wcpt_h = WeightedColoredPoint(
        name="H", x=np.float64(1.8), y=np.float64(2.25), color="red", weight=np.float64(125.0)
    )

    shapes: list[Shape] = [wp_d, c, poly, pt_a, pt_b, wp_e, cpt_f, wcpt_h]
    points: list[Point] = []
    for s in shapes:
        match s:
